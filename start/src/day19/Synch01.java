...
public class Synch01 {
	int[] num =new int[5];		//	 힙영역에 만들어짐 -> 자동 초기화
	
	public Synch01() {
//		super();	//	암묵적으로 상위클래스의 기본 생성자 호출이 된 상태이다.
		/*
		 	this	:	현재 실행중인 자기 자신의 주소를 기억하는 예약된 변수
		 	super 	:	상위클래스의 주소를 기억하는 예약된 변수
		 */
		NumThread01 t1 = new NumThread01(this);	//	NumThread01() 로 할 경우 어떤 건지 찾지 못하기 때문에 this로 자신의 주소를 넘겨준다.
	...
	public void run() {
		...
		synchronized(main.num) {
			for(int i = 0 ; i < 5 ; i++) {
				...
				// 잠시 쉬는 시간을 랜덤하게 주도록 한다. 0.5 ~ 2초 를 랜덤하게
...
class NumThread02 extends Thread {
	...
	public void run() {
		...
		synchronized(main.num) {
			//	main 의 num 배열을 사용하는 동안은 다른 곳에서 배열을 사용하면 안 된다.
			for(int i = 0 ; i < 5 ; i++) {
				int no = main.num[i];
				//	숫자를 꺼냈으므로 출력한다.
