...
/*
 	파라미터로 전달된 문자열
 		http://www.blackpink.org/member/@id=lhj&pw=11111&tel=01011112222
 	중에서 넘겨진 데이터만 잘라서 출력하시오.
 */

public class Test01 {

	public Test01() {
		String url = "http://www.blackpink.org/member/?@id=lhj&pw=11111&tel=01011112222";
//		StringTokenizer info = new StringTokenizer(url, "?");
		// 위 문자열에서 데이터부분만 추출하려면
		// ? 기호의 위치값을 알아낸 후 그 바로 다음 위치에서부터 끝까지 잘라내면 된다.
		int idx = url.indexOf('?');
		String param = url.substring(idx+1);	//	==>	@id=lhj&pw=11111&tel=01011112222
...
//		}
		for(int i = 0 ; token.hasMoreTokens() /*i < cnt*/ ; i++) {
			tmp[i] = token.nextToken();	// 이 명령을 실행하는 순간 버퍼메모리에서 꺼낸 데이터는 삭제된다.
		}
		/*
...
//			
//			for(int j = 0 ; j < data[cnt].length ; j++) {
//				data[i][j] = tk.nextToken();
//			}
		}	
		for(String[] arr : data) {
