...
public class Nemo {
	...
	public Nemo() {
		...
		// JVM 이 이 클래스를 객체로 만드는 순간 
		// 기본 생성자를 만들어서 사용하게 된다.
		// 만약 생성자를 하나라도 만들면
		// JVM 이 기본 생성자를 만들지 않는다.
		// 따라서 생성자를 정의를 할 경우는 반드시 내용이 비어있는 기본생성자도 정의하는 것이 좋다.
	...
	public void setArea() {
		...
		// 따라서 이미 필요한 데이터가 준비가 되어있고 접근이 가능하므로 
		// 따로 데이터를 전달받지 않아도 된다.
		area = width * height;
	}
	public int getWidth() {
	...
	// 이 클래스의 내용을 문자열로 반환해는 함수 toString()
	// 이 함수는 출력할 때 자동 호출되는 함수
	@Override
	...
	public boolean equals(Object o) {
		...
		try {
			...
			// 만약 입력된 객체가 Nemo 로 강제형변환할 때 예외가 발생하면
			// 이 작업 이후의 모든 작업들은 실행자체가 무의미해진다.
			// 따라서 이 함수의 결과는 "다르다"가 될 것이고
