...
/*
 	이 클래스를 사용한다는 것은
 	이 클래스를 new 시킨다는 것이고
 	의미는 오라클에 접속해서 scott 계정이 가지고 있는 
 	emp 테이블의 내용을 조회, 수정, 삭제... 등의 작업을 한다는 것이다.
 	
 	따라서 이 클래스가 객체가 될 때는
 	오라클을 사용할 준비가 되어있어야 한다.
 	이 말은 드라이버 로딩이 되어있어야 한다는 뜻.
 */
	...
	public EmpDao02() {
		db = new ScottJDBC();
		//	이 순간 ScottJDBC 의 인스턴스가 만들어지면 오라클 드라이버를 로딩한 상태가 된다.
